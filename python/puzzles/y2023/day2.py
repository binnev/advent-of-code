...
def part1(input: str):
    ...
    result = 0
    games = _parse_games(input)
    for ii, game in games:
        if _is_game_possible(game, limit):
            result += ii
    return result
...
def _parse_game(game: str) -> tuple[int, list[RgbTuple]]:
...
def _parse_games(input: str) -> list[tuple[int, list[RgbTuple]]]:
    return [_parse_game(line) for line in input.splitlines()]


def _is_game_possible(game: list[RgbTuple], limit: RgbTuple) -> bool:
    for hand in game:
